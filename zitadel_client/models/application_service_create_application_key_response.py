...
class ApplicationServiceCreateApplicationKeyResponse(BaseModel):
    ...
    key_id: Optional[StrictStr] = Field(
        default=None,
        alias="keyId",
        description="The unique ID of the newly created application key.",
    )
    ...
    key_details: Optional[Base64Bytes] = Field(
        default=None,
        alias="keyDetails",
        description="KeyDetails contains the serialized private key and additional information depending on the key type.  It must be stored safely, as it will not be possible to retrieve it again.  A new key can be generated using the CreateApplicationKey endpoint.",
    )
...
ApplicationServiceCreateApplicationKeyResponse.model_rebuild(raise_errors=False)
