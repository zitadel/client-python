...
none_type = type(None)
SESSIONSERVICEVALUE_ONE_OF_SCHEMAS = ["Dict[str, object]", "List[str]", "bool", "float", "none_type", "str"]

class SessionServiceValue(BaseModel):
    """
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = SessionServiceValue.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in SessionServiceValue with oneOf schemas: Dict[str, object], List[str], bool, float, none_type, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in SessionServiceValue with oneOf schemas: Dict[str, object], List[str], bool, float, none_type, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into SessionServiceValue with oneOf schemas: Dict[str, object], List[str], bool, float, none_type, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into SessionServiceValue with oneOf schemas: Dict[str, object], List[str], bool, float, none_type, str. Details: " + ", ".join(error_messages))
        else:
            return instance
