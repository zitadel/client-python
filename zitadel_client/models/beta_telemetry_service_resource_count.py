...
class BetaTelemetryServiceResourceCount(BaseModel):
    ...
    instance_id: Optional[StrictStr] = Field(
        default=None,
        alias="instanceId",
        description="The ID of the instance for which the resource counts are reported.",
    )
    parent_type: Optional[BetaTelemetryServiceCountParentType] = Field(
        default=None, alias="parentType"
    )
    parent_id: Optional[StrictStr] = Field(
        default=None,
        alias="parentId",
        description="The parent ID of the resource counts (e.g. organization or instance ID).  For example, reporting the amount of users per organization would use  `COUNT_PARENT_TYPE_ORGANIZATION` as parent type and the organization ID as parent ID.",
    )
    resource_name: Optional[StrictStr] = Field(
        default=None,
        alias="resourceName",
        description="The resource counts to report, e.g. amount of `users`, `organizations`, etc.",
    )
    table_name: Optional[StrictStr] = Field(
        default=None,
        alias="tableName",
        description="The name of the table in the database, which was used to calculate the counts.  This can be used to deduplicate counts in case of multiple reports.  For example, if the counts were calculated from the `users14` table,  the table name would be `users14`, where there could also be a `users15` table  reported at the same time as the system is rolling out a new version.",
    )
    ...
    amount: Optional[StrictInt] = Field(
        default=None, alias="amount", description="The actual amount of the resource."
    )
...
BetaTelemetryServiceResourceCount.model_rebuild(raise_errors=False)
