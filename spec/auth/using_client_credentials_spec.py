...
class TestUseClientCredentialsSpec:
    ...
    def generate_user_secret(
        ...
        if user_id_response.status < 400:
            user_response_map: Dict[str, Any] = json.loads(
                user_id_response.data.decode("utf-8")
            )
            user_payload: Any = user_response_map.get("user")
            user_id: Any = (
                user_payload.get("id") if isinstance(user_payload, dict) else None
            )
            ...
            if isinstance(user_id, str) and user_id:
                ...
                if secret_response.status < 400:
                    secret_data: Dict[str, Any] = json.loads(
                        secret_response.data.decode("utf-8")
                    )
                    ...
                    raise Exception(
                        f"API call to generate secret failed for user ID: '{user_id}'. Response: {error_body}"
                    )
            else:
                print(user_response_map)  # noqa T201
                raise ValueError(
                    f"Could not parse a valid user ID from API response for login name: '{login_name}'."
                )
        else:
            error_body = user_id_response.data.decode("utf-8")
            raise Exception(
                f"API call to retrieve user failed for login name: '{login_name}'. Response: {error_body}"
            )

    async def test_retrieves_general_settings_with_valid_client_credentials(
