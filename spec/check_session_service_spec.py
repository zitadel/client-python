...
async def session(
    ...
    await client.user_service.add_human_user(request1)
    ...
    response = await client.session_service.create_session(request)
    yield response
    # Teardown
    ...
    try:
        await client.session_service.delete_session(
            delete_body,
        )
    except ApiException:
        pass
...
class TestSessionServiceSanityCheckSpec:
    ...
    async def test_retrieves_session_details_by_id(
        ...
        response: SessionServiceGetSessionResponse = (
            await client.session_service.get_session(request)
        )
    ...
    async def test_includes_created_session_when_listing(
        ...
        response = await client.session_service.list_sessions(request)
    ...
    async def test_updates_session_lifetime_and_returns_new_token(
        ...
        response = await client.session_service.set_session(request)
